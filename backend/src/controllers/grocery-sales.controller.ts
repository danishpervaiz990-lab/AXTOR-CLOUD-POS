...
async function captureAndPost(req: Request, documentId: string) {
  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (!snapshot.version) {
      ...
      document.metadata = { ...metadata, groceryCostSnapshot: snapshot };
...
export async function groceryCreateSale(req: Request, res: Response) {
  ...
  try {
    const posted = await captureAndPost(req, payload.data.id);
    if (posted?.snapshot) payload.data.metadata = { ...(payload.data.metadata || {}), groceryCostSnapshot: posted.snapshot };
    payload.data.accounting = posted?.accounting || null;
  } catch (error: any) {
    console.error("grocery sale accounting error:", error);
    payload.accountingWarning = `Sale posted, accounting synchronization failed: ${error?.message || "unknown error"}`;
  }
