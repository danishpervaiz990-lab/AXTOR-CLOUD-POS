...
function formatSalesReturn(row: any) {
  ...
  return {
    ...
    refundedAmount: Number(row.refundAmount || 0),
  ...
  try {
    ...
    const result = await (prisma as any).$transaction(async (tx: any) => {
      ...
      for (let index = 0; index < preparedItems.length; index += 1) {
        ...
        const beforeGlobalQty = Number(product.currentStock || 0);
        await tx.product.update({
          where: { id: item.productId },
          data: { currentStock: { increment: item.returnQty } },
        });

        const warehouseId = sourceDoc.warehouseId || cleanString(req.body?.warehouseId) || null;
        let beforeQty = beforeGlobalQty;
        let afterQty = roundQty(beforeGlobalQty + item.returnQty);

        if (warehouseId) {
          let warehouseStock = await tx.inventoryStock.findFirst({
            where: { businessId, warehouseId, productId: item.productId },
          });
          if (!warehouseStock) {
            ...
            warehouseStock = await tx.inventoryStock.upsert({
              where: {
              ...
              create: {
                ...
                productId: item.productId,
                warehouseId,
                qtyOnHand: anyAllocatedStock ? 0 : beforeGlobalQty,
                qtyReserved: 0,
              },
              update: {},
            });
          }
          beforeQty = Number(warehouseStock.qtyOnHand || 0);
          afterQty = roundQty(beforeQty + item.returnQty);
          await tx.inventoryStock.update({
            where: { id: warehouseStock.id },
            data: { qtyOnHand: { increment: item.returnQty } },
          });
        }
        ...
        await tx.stockMovement.create({
          data: {
            businessId,
            movementNo: `${returnNo}-${String(index + 1).padStart(3, "0")}-${Date.now()}`,
            productId: item.productId,
            sku: item.sku,
            productName: item.name,
            warehouseId,
      ...
      await writeAudit(tx, req, {
        ...
        before: { returnedAmount: previousReturnedAmount, returnCount: Number(sourceDoc.returnCount || 0) },
        after: { returnNo, returnedAmount: nextReturnedAmount, returnCount: nextReturnCount, returnStatus: nextReturnStatus },
      });

      return { salesReturn, duplicate: false };
    });

    return res.status(result.duplicate ? 200 : 201).json({
      ok: true,
      duplicate: Boolean(result.duplicate),
      message: result.duplicate ? "Duplicate return request ignored" : "Sales return posted successfully",
      data: formatSalesReturn(result.salesReturn),
    });
  } catch (error: any) {
