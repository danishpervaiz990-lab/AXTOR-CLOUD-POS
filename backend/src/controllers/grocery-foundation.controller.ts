...
export async function grocerySalesGuard(req: Request, res: Response, next: NextFunction) {
  try {
    const t = tenant(req); const access = await loadUserAccess(db, t.businessId, t.userId); const input = req.body || {}; const items: any[] = Array.isArray(input.items) ? input.items : [];
    const productIds: string[] = [...new Set<string>(items.map((i: any) => text(i.productId)).filter(Boolean))];
    const products: any[] = productIds.length ? await db.product.findMany({ where: { businessId: t.businessId, id: { in: productIds }, active: true, deleted: false }, select: { id: true, price: true } }) : [];
    const priceMap: Map<string, number> = new Map(products.map((p: any): [string, number] => [String(p.id), num(p.price)]));
    const hasPriceOverride = items.some((i: any) => { const base = priceMap.get(text(i.productId)); const supplied = num(i.unitPrice ?? i.rate ?? i.price, base ?? 0); return base !== undefined && Math.abs(supplied - base) > .001; });
    ...
    const customerId = text(input.customerId); const paid = round2((Array.isArray(input.paymentLines) ? input.paymentLines : []).reduce((s: number, p: any) => s + Math.max(0, num(p.amount)), 0)); let total = 0; for (const i of items) { const qty = Math.max(0, num(i.quantity ?? i.qty)); const fallbackRate = priceMap.get(text(i.productId)) ?? 0; const rate = num(i.unitPrice ?? i.rate ?? i.price, fallbackRate); const discount = Math.max(0, num(i.discountAmount ?? i.discount)); const taxable = Math.max(0, qty * rate - discount); total += taxable + taxable * Math.max(0, num(i.taxRate)) / 100; } total = round2(Math.max(0, total - Math.max(0, num(input.discount ?? input.discountTotal)))); const balance = round2(Math.max(0, total - paid));
    if (balance > 0) {
