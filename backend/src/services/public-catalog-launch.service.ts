...
import {
  ONBOARDING_READY_INDUSTRY_CODES,
  industryLaunchReadiness,
} from "../industry/activate-launch-ready-packs.js";
import {
  getIndustryPack,
...
function publicPack(pack: IndustryPack) {
  ...
  return {
    ...
    availabilityMessage: canRegister
      ...
      : "Onboarding is temporarily unavailable because launch-readiness validation failed",
...
function assertPackReady(industryCode: string): IndustryPack {
  ...
  if (!onboardingCodes.has(industryCode) || !readiness.ready) {
    throw new PublicCatalogError(
      409,
      "INDUSTRY_NOT_LAUNCH_READY",
      `${pack.name} cannot be onboarded until all launch-readiness checks pass`,
...
async function createIndustryRoles(tx: Prisma.TransactionClient, businessId: string, pack: IndustryPack): Promise<number> {
  ...
  for (const [name, permissions] of Object.entries(pack.defaultRoles)) {
    if (!name.trim() || !permissions.length || name.trim().toLowerCase() === "owner") continue;
    await tx.role.create({
      data: {
        businessId,
        name: name.trim(),
        description: `Default ${pack.name} operational role`,
        isSystemRole: true,
...
async function createIndustrySettings(
  ...
  const values: Array<[string, Prisma.InputJsonValue]> = [
    ...
    ["industry.launch", {
      registryVersion: INDUSTRY_REGISTRY_VERSION,
      operationalStatus: pack.operationalStatus,
      registrationEnabled: pack.registrationEnabled,
      activatedAt: new Date().toISOString(),
    } as Prisma.InputJsonValue],
  ];

  for (const [key, value] of values) {
    await tx.industrySetting.create({ data: { businessId, key, value } });
  }
}
...
export async function register(req: Request, input: Record<string, unknown>) {
  ...
  try {
    ...
    throw new PublicCatalogError(400, "VALIDATION_ERROR", "Timezone is invalid");
  }
  ...
  try {
    return await prisma.$transaction(async tx => {
      ...
      const branch = await tx.branch.create({
        data: { businessId: business.id, name: firstBranch, code: "MAIN", country, type: pack.name },
      });
      const warehouse = await tx.warehouse.create({
        data: { businessId: business.id, branchId: branch.id, name: firstWarehouse, code: "MAIN" },
      });
      ...
      await tx.counter.create({
        data: { businessId: business.id, branchId: branch.id, name: firstCounter, code: "POS-1", cashierUserId: owner.id },
      });
      ...
      await tx.businessCurrency.create({
        data: { businessId: business.id, currencyCode: baseCurrency, isBase: true, active: true },
      });
      await tx.businessTaxSetting.create({
        data: {
          ...
          taxSystem,
          taxLabel,
          registrationNumber: taxRegistrationNumber,
          pricesIncludeTax: input.pricesIncludeTax === true,
        },
      });
      await tx.businessLocale.create({
        data: {
          ...
          timezone,
          dateFormat: String(input.dateFormat || "yyyy-MM-dd"),
          numberLocale: String(input.numberLocale || "en-QA"),
        },
      });
      ...
      for (const profile of profileRows) {
        await tx.printProfile.create({
          data: {
            ...
            config: { fields: pack.printFields, browserPrint: true, industryCode: pack.code },
      ...
      const launchEvidence = {
        ...
        formSchemaCount: pack.entities.length,
        notificationRuleCount: pack.notificationRules.length,
        printFieldCount: pack.printFields.length,
        sampleDataRequested: input.sampleDataRequested === true,
      };
      ...
      await tx.tenantProvisioningRun.create({
        data: {
          businessId: business.id,
          idempotencyKey,
