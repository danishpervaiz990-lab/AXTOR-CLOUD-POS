...
/**
 * Commercial plans are intentionally non-enforcing while Axtor is in product
 * development. Set AXTOR_ENFORCE_SUBSCRIPTION_PLANS=true when paid plan splits
 * are ready to be launched. Authentication, tenant isolation and role
 * permissions remain authoritative in both modes.
 */
export function subscriptionPlansEnforced(): boolean {
  return flag(process.env.AXTOR_ENFORCE_SUBSCRIPTION_PLANS);
}
...
export async function loadEntitlements(tx: any, businessId: string): Promise<EntitlementContext> {
  if (tx === prisma) {
    const cached = entitlementCache.get(businessId);
  ...
  const enforcementEnabled = subscriptionPlansEnforced();
  ...
  let readOnly = Boolean(business.maintenanceMode) || businessBlocked;

  if (enforcementEnabled) {
    readOnly = readOnly || ["SUSPENDED", "CANCELLED", "EXPIRED"].includes(status) || Boolean(periodExpired && !graceActive);
  } else {
    features["*"] = { enabled: true, limit: null };
    Object.keys(limits).forEach((name) => { limits[name] = null; });
  }

  const result = { plan, subscription, features, limits, readOnly, enforcementEnabled };
  if (tx === prisma) entitlementCache.set(businessId, { expiresAt: Date.now() + 10_000, value: result });
  return result;
}

export function clearEntitlementCache(businessId?: string): void {
...
export async function assertUsageLimit(tx: any, businessId: string, resource: keyof EntitlementContext["limits"], increment = 1): Promise<void> {
  if (!subscriptionPlansEnforced()) return;
  const context = await loadEntitlements(tx, businessId);
  const limit = context.limits[resource];
  if (limit === null) return;
  const model = resource === "users" ? tx.user
    : resource === "branches" ? tx.branch
      : resource === "warehouses" ? tx.warehouse
        : resource === "currencies" ? tx.businessCurrency
          : null;
  if (!model) return;
  const count = await model.count({
    where: resource === "users"
      ? { businessId, status: { not: "DISABLED" } }
      : { businessId, ...(resource === "branches" || resource === "warehouses" ? { active: true } : {}) },
  });
...
function featureAllowed(context: EntitlementContext, featureKey: string): boolean {
  if (!context.enforcementEnabled || context.features["*"]?.enabled) return true;
  if (context.features[featureKey]?.enabled) return true;
  const segments = String(featureKey || "").split(".").filter(Boolean);
  while (segments.length > 1) {
    segments.pop();
    if (context.features[`${segments.join(".")}.*`]?.enabled) return true;
  }
  return false;
}
...
export function requireFeature(featureKey: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (context.readOnly && req.method !== "GET" && req.method !== "HEAD") throw new ApiError(402, "Business is read-only. Contact the platform administrator.");
      if (!featureAllowed(context, featureKey)) throw new ApiError(403, "Feature is not included in the current subscription", { featureKey, upgradeRequired: true });
      next();
    } catch (error) {
