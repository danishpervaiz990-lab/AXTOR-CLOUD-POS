...
async function enforceCustomerCreditHold(tx: any, businessId: string, access: UserAccess, customer: any) {
  if (!customer) return;
  const now = new Date();
  const [overdueCount] = await Promise.all([
    tx.salesDocument.count({ where: { businessId, customerId: customer.id, documentType: "INVOICE", balance: { gt: 0 }, dueDate: { lt: now }, status: { notIn: ["DRAFT", "CANCELLED", "VOID"] } } }),
  ]);
  const currentBalance = roundMoney(Number(customer.balance || 0));
  const creditLimit = roundMoney(Number(customer.creditLimit || 0));
  const overLimit = creditLimit > 0 && currentBalance >= creditLimit - 0.001;
  if ((overdueCount > 0 || overLimit) && !hasPermission(access, "sales_documents.override_customer_credit_hold")) {
    const reasons = [overdueCount > 0 ? "overdue invoice" + (overdueCount > 1 ? "s" : "") : "", overLimit ? "credit limit reached" : ""].filter(Boolean).join(" and ");
    throw new Error(`Customer is on credit hold (${reasons}). Ask an Owner or Manager to grant the customer credit-hold override permission.`);
  }
}
...
export async function createSalesDocument(req: Request, res: Response) {
  try {
    ...
    const result = await (prisma as any).$transaction(async (tx: any) => {
      ...
      if (postingMode === "post" && documentType === "INVOICE") await enforceCustomerCreditHold(tx, businessId, access, customerInfo.customer);
...
export async function postSalesDocument(req: Request, res: Response) {
  try {
    ...
    const result = await (prisma as any).$transaction(async (tx: any) => {
      ...
      if (document.documentType === "INVOICE" && document.customerId) {
        const customerForHold = await tx.customer.findFirst({ where: { id: document.customerId, businessId } });
        await enforceCustomerCreditHold(tx, businessId, access, customerForHold);
      }
