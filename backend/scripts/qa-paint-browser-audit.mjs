...
async function inspectPaintPage(page, key, terms) {
  return page.evaluate(({ key, terms }) => {
    ...
    const notFound = /\bpage not found\b/i.test(pageIdentity)
      || /(^|\n)\s*404(?:\s|$)/i.test(pageIdentity);
    return {
      key,
      ok: hasTerms && Boolean(app) && body.trim().length > 20 && !notFound,
      hasTerms,
      notFound,
...
async function auditPage(context, user, key, route, terms) {
  ...
  return {
    ...
    branchOk,
    industryOk,
    hasTerms: verification.hasTerms,
  ...
  for (const user of auditUsers) {
    ...
    try {
      for (const [key, route, terms] of pages) {
        ...
        console.log('Paint browser page', { user: user.key, role: user.role, key, ok: entry.ok, http: entry.http, error: entry.error || null, hasTerms: entry.hasTerms, notFound: entry.notFound });
