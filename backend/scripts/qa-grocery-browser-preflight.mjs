...
try {
  ...
  const visible = await Promise.all([
    page.locator('#loginEmail').isVisible().catch(() => false),
    page.locator('#loginPassword').isVisible().catch(() => false),
    page.locator('#loginButton').isVisible().catch(() => false),
  ]);
  await page.screenshot({ path: `${evidenceDir}/login-page.png`, fullPage: true });
  results.push({
    name: 'Live login form',
    pass: response?.status() === 200 && Object.values(fields).every((value) => value === 1) && visible.every(Boolean),
    details: fields,
  });
  await context.close();
} catch (error) {
...
const meaningfulErrors = errors.filter((message) => !/favicon|robots\.txt|ERR_ABORTED|chrome-extension|Failed to load resource.*404/i.test(message));
results.push({ name: 'No critical browser errors', pass: meaningfulErrors.length === 0, errors: meaningfulErrors });
const report = {
  generatedAt: new Date().toISOString(),
  publicOrigin,
  note: 'Route and industry-branch headers are enforced separately by the HTTP preflight.',
  results,
