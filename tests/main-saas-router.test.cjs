...
assert.match(login,/const LAST_EMAIL='axtorLastLoginEmail'/);
assert.match(login,/workspace\.value=normalizeEmail\(email\.value\)/);
assert.match(login,/businessSlug:loginEmail,email:loginEmail/);
assert.match(login,/\/api\/v1\/auth\/me/);
assert.match(login,/sessionStorage\.getItem\('axtorAuthReturnUrl'\)\|\|'router\.html'/);
assert.match(login,/mustChangePassword/);
assert.match(login,/change-password\.html/);
assert.doesNotMatch(login,/getRegistrations\(\)/);
assert.doesNotMatch(login,/caches\.keys\(\)/);
