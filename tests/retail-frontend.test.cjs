...
assert.match(app,/Financial report requests were not sent/);
assert.match(app,/async function loadOperationalDashboard\(\)[\s\S]*\/api\/v1\/dashboard\/summary[\s\S]*\/api\/v1\/products\?active=true[\s\S]*\/api\/v1\/customers\?active=true/);
assert.doesNotMatch(app.match(/async function loadOperationalDashboard\(\)[\s\S]*?\n  }/)[0],/\/api\/v1\/reports\//);
...
assert.match(dashboard,/retail-app\.js\?v=20260803-role-aware1/);
...
console.log('PASS: isolated General Retail frontend, role-aware dashboard reports, guarded transaction flows and unified A4\/80mm\/58mm printing');
