...
assert.match(proxy, /data-axtor-development-runtime="20260804-strict2"/);
assert.match(proxy, /AXTOR_DEVELOPMENT_MODE=true/);
assert.match(proxy, /\.axtor-plan-block/);
assert.match(proxy, /lower\.indexOf\("trial:"\)===0/);
assert.match(proxy, /lower\.indexOf\("day\(s\) remaining"\)>0/);
assert.match(proxy, /plans & subscription/);
...
console.log('PASS: strict tenant-industry correction, reliable development plan/trial removal, canvas lifecycle guard and asset caching are gateway-certified');
