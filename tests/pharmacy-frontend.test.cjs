...
const billingGuard=fs.readFileSync(path.join(root,'js','pharmacy-billing-data-guard.js'),'utf8');
const documents=fs.readFileSync(path.join(root,'js','pharmacy-document-routing.js'),'utf8');
const billingHtml=fs.readFileSync(path.join(root,'pharmacy-billing.html'),'utf8');
...
for(const token of ['/api/v1/settings','/api/v1/commercial/context','settings.view','hasPermission','invoice.settings','defaultPrintSize','showPrescriptionReference','showPharmacist','showBatchExpiry'])assert.ok(printSettings.includes(token),`print settings missing ${token}`);
assert.ok(printSettings.includes('if(!await mayReadSettings())'),'restricted print settings must use cache without requesting settings');
assert.ok(billingGuard.includes('data-pharmacy-page')||billingGuard.includes('dataset?.pharmacyPage'),'billing guard page scope missing');
assert.ok(billingGuard.includes('/api/v1/customers'),'billing guard customer endpoint missing');
assert.ok(billingGuard.includes('return original(path,options)'),'billing guard must preserve all other API requests');
assert.ok(billingHtml.includes('pharmacy-billing-data-guard.js?v=20260804-role-aware1'),'billing guard is not activated before Pharmacy runtime');
assert.ok(billingHtml.indexOf('pharmacy-billing-data-guard.js')<billingHtml.indexOf('pharmacy-app.js'),'billing guard must load before Pharmacy runtime');
assert.ok(billingHtml.includes('pharmacy-print-settings-backend.js?v=20260804-role-aware1'),'role-aware print settings cache marker missing');
...
console.log(`PASS: ${expected.length} Pharmacy pages with FEFO, prescription safety, role-aware billing/settings requests and shared document routing.`);
