...
const roleAware=fs.readFileSync(path.join(root,'js','pharmacy-role-aware-api.js'),'utf8');
...
for(const token of ['dataset?.pharmacyPage','pharmacist','customers','Promise.resolve({ok:true,data:[]})'])assert.ok(roleAware.includes(token),`role-aware adapter missing ${token}`);
assert.ok(roleAware.includes('^\\/api\\/v1\\/customers'),'role-aware adapter missing tenant customer-route guard');
...
const billing=fs.readFileSync(path.join(root,'pharmacy-billing.html'),'utf8');
assert.ok(billing.includes('pharmacy-role-aware-api.js'),'pharmacy-billing.html: pharmacist-safe adapter missing');
assert.ok(billing.indexOf('pharmacy-role-aware-api.js')<billing.indexOf('pharmacy-app.js'),'pharmacy-billing.html: adapter must load before Pharmacy runtime');
...
console.log(`PASS: ${expected.length} Pharmacy pages with FEFO, prescription safety, role-aware loading, tenant print settings and shared document routing.`);
