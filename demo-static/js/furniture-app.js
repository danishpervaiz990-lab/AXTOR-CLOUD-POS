(function () {
  ...
  function fieldHtml(field) {
    ...
    if (field.type === "textarea") {
      return '<div><label>' + esc(field.label) + '</label><textarea name="' + field.name + '"' + required + '></textarea></div>';
    }
  ...
  function formPanel(id, title, fields, button) {
    return '<section class="f-panel"><h2>' + esc(title) + '</h2><form id="' + id + '" class="f-form">' + fields.map(fieldHtml).join("") + '<div class="f-actions"><button class="f-btn" type="submit">' + esc(button || "Save") + '</button></div></form><div id="' + id + 'Status" class="f-status"></div></section>';
  }

  function tablePanel(id, title, columns) {
    return '<section class="f-panel"><div class="f-toolbar"><h2>' + esc(title) + '</h2><input class="f-search" data-search="' + id + '" placeholder="Search records"></div><div class="f-table-wrap"><table class="f-table"><thead><tr>' + columns.map(function (column) {
      return "<th>" + esc(column[1]) + "</th>";
    }).join("") + '</tr></thead><tbody id="' + id + '"></tbody></table></div></section>';
  }
  ...
  async function dashboard() {
    shell("Furniture Studio Dashboard", "Custom orders, production, delivery and receivables");
    const metrics = await request("GET", "/dashboard");
    document.getElementById("app").innerHTML = '<div class="f-kpis"><div class="f-kpi"><span>Open Orders</span><strong>' + esc(metrics.openOrders || 0) + '</strong></div><div class="f-kpi"><span>In Production</span><strong>' + esc(metrics.inProduction || 0) + '</strong></div><div class="f-kpi"><span>Scheduled Deliveries</span><strong>' + esc(metrics.scheduledDeliveries || 0) + '</strong></div><div class="f-kpi"><span>Outstanding Balance</span><strong>' + esc(metrics.outstandingBalance || 0) + '</strong></div></div><section class="f-panel"><h2>Order Lifecycle</h2><div class="f-flow"><div class="f-step">Design</div><div class="f-step">Cutting</div><div class="f-step">Assembly</div><div class="f-step">Finishing</div><div class="f-step">Delivery</div></div></section>';
  }

  async function orders() {
    shell("Custom Orders", "Quoted jobs with deposits, balances and due dates");
    const columns = [["orderNo", "Order"], ["customerId", "Customer"], ["description", "Description"], ["quotedAmount", "Quoted"], ["balance", "Balance"], ["expectedAt", "Expected"], ["status", "Status"]];
    document.getElementById("app").innerHTML = formPanel("orderForm", "Create Custom Order", [
      ...
      { name: "depositRequired", label: "Deposit required", type: "number" },
      { name: "expectedAt", label: "Expected date", type: "date" }
    ...
    bind("orderForm", "POST", "/orders", null, load, true);
    await load();
  }

  const ACTIONS = {
    measurements: { title: "Measurements", subtitle: "Order-specific dimensional records", id: "measurementForm", button: "Save Measurement", fields: [{ name: "orderId", label: "Order ID", type: "text", required: true }, { name: "measurementKey", label: "Measurement", type: "text", required: true }, { name: "measurementValue", label: "Value", type: "number", required: true }, { name: "unit", label: "Unit", type: "text" }, { name: "notes", label: "Notes", type: "textarea" }], method: "POST", path: function (body) { const id = body.orderId; delete body.orderId; return "/orders/" + encodeURIComponent(id) + "/measurements"; } },
    production: { title: "Production Stages", subtitle: "Assign and progress manufacturing stages", id: "stageForm", button: "Update Stage", fields: [{ name: "stageId", label: "Stage ID", type: "text", required: true }, { name: "status", label: "Status", type: "select", required: true, options: ["pending", "in_progress", "completed", "blocked"] }, { name: "assignedTo", label: "Assigned to", type: "text" }, { name: "notes", label: "Notes", type: "textarea" }], method: "PATCH", path: function (body) { const id = body.stageId; delete body.stageId; return "/production-stages/" + encodeURIComponent(id); } },
    approvals: { title: "Design Approvals", subtitle: "Revision-based customer and design approvals", id: "approvalForm", button: "Record Approval", fields: [{ name: "orderId", label: "Order ID", type: "text", required: true }, { name: "status", label: "Status", type: "select", required: true, options: ["approved", "rejected", "changes_requested"] }, { name: "approvedBy", label: "Approved by", type: "text" }, { name: "notes", label: "Notes", type: "textarea" }], method: "POST", path: function (body) { const id = body.orderId; delete body.orderId; return "/orders/" + encodeURIComponent(id) + "/approvals"; } },
    payments: { title: "Order Payments", subtitle: "Deposit and installment allocation", id: "paymentForm", button: "Post Payment", fields: [{ name: "orderId", label: "Order ID", type: "text", required: true }, { name: "amount", label: "Amount", type: "number", required: true }, { name: "paymentType", label: "Payment type", type: "select", options: ["deposit", "installment", "final"] }, { name: "method", label: "Method", type: "select", options: ["cash", "card", "bank_transfer", "cheque"] }, { name: "reference", label: "Reference", type: "text" }], method: "POST", idempotent: true, path: function (body) { const id = body.orderId; delete body.orderId; return "/orders/" + encodeURIComponent(id) + "/payments"; } },
    deliveries: { title: "Furniture Deliveries", subtitle: "Schedule transport for completed orders", id: "deliveryForm", button: "Schedule Delivery", fields: [{ name: "customOrderId", label: "Order ID", type: "text", required: true }, { name: "scheduledAt", label: "Scheduled at", type: "datetime-local", required: true }, { name: "vehicleReference", label: "Vehicle", type: "text" }, { name: "driverName", label: "Driver", type: "text" }], method: "POST", path: "/deliveries" },
    procurement: { title: "Order Procurement", subtitle: "Order-linked material and supplier requirements", id: "procurementForm", button: "Create Procurement", fields: [{ name: "customOrderId", label: "Order ID", type: "text", required: true }, { name: "supplierId", label: "Supplier ID", type: "text" }, { name: "description", label: "Description", type: "textarea", required: true }, { name: "amount", label: "Amount", type: "number" }, { name: "status", label: "Status", type: "select", options: ["requested", "ordered", "received"] }], method: "POST", path: "/procurements" },
    settings: { title: "Furniture Settings", subtitle: "Order, delivery and warranty notification rules", id: "ruleForm", button: "Save Rule", fields: [{ name: "eventKey", label: "Event key", type: "text", required: true }, { name: "channel", label: "Channel", type: "select", options: ["in_app", "email", "sms", "whatsapp"] }, { name: "daysBefore", label: "Days before", type: "number" }, { name: "active", label: "Active", type: "select", options: ["true", "false"] }], method: "PUT", path: "/notification-rules/furniture" }
  };

  async function actionPage(config) {
    shell(config.title, config.subtitle);
    document.getElementById("app").innerHTML = formPanel(config.id, config.button, config.fields, config.button) + '<section class="f-panel"><div class="f-note">All operations are tenant-scoped and protected by server-side permissions.</div></section>';
    bind(config.id, config.method, config.path, null, null, Boolean(config.idempotent));
  }

  async function installations() {
    shell("Installations", "Schedule, complete and capture customer sign-off");
    document.getElementById("app").innerHTML = formPanel("createForm", "Schedule Installation", [{ name: "deliveryId", label: "Delivery ID", type: "text", required: true }, { name: "scheduledAt", label: "Scheduled at", type: "datetime-local", required: true }, { name: "technicianName", label: "Technician", type: "text" }, { name: "notes", label: "Notes", type: "textarea" }]) + formPanel("completeForm", "Complete Installation", [{ name: "installationId", label: "Installation ID", type: "text", required: true }, { name: "notes", label: "Completion notes", type: "textarea" }]) + formPanel("signoffForm", "Customer Sign-off", [{ name: "installationId", label: "Installation ID", type: "text", required: true }, { name: "customerName", label: "Customer name", type: "text", required: true }, { name: "signatureReference", label: "Signature reference", type: "text" }, { name: "notes", label: "Notes", type: "textarea" }]);
    bind("createForm", "POST", "/installations");
    bind("completeForm", "PATCH", function (body) { const id = body.installationId; delete body.installationId; return "/installations/" + encodeURIComponent(id) + "/complete"; });
    bind("signoffForm", "POST", function (body) { const id = body.installationId; delete body.installationId; return "/installations/" + encodeURIComponent(id) + "/signoff"; });
  }

  async function returnsPage() {
    shell("Returns & Resolution", "Record custom-order returns and controlled resolutions");
    document.getElementById("app").innerHTML = formPanel("createForm", "Create Return", [{ name: "customOrderId", label: "Order ID", type: "text", required: true }, { name: "reason", label: "Reason", type: "textarea", required: true }]) + formPanel("resolveForm", "Resolve Return", [{ name: "returnId", label: "Return ID", type: "text", required: true }, { name: "resolution", label: "Resolution", type: "textarea", required: true }]);
    bind("createForm", "POST", "/returns");
    bind("resolveForm", "PATCH", function (body) { const id = body.returnId; delete body.returnId; return "/returns/" + encodeURIComponent(id) + "/resolve"; });
  }

  async function warranty() {
    shell("Warranty Claims", "Register and resolve customer warranty claims");
    document.getElementById("app").innerHTML = formPanel("claimForm", "Create Claim", [{ name: "warrantyId", label: "Warranty ID", type: "text", required: true }, { name: "issue", label: "Issue", type: "textarea", required: true }]) + formPanel("resolveForm", "Resolve Claim", [{ name: "claimId", label: "Claim ID", type: "text", required: true }, { name: "resolution", label: "Resolution", type: "textarea", required: true }]);
    bind("claimForm", "POST", "/warranty-claims");
    bind("resolveForm", "PATCH", function (body) { const id = body.claimId; delete body.claimId; return "/warranty-claims/" + encodeURIComponent(id) + "/resolve"; });
  }

  async function reports() {
    shell("Furniture Reports", "Orders, production, claims and payment summaries");
    document.getElementById("app").innerHTML = '<section class="f-panel"><button id="run" class="f-btn">Refresh Report</button><pre id="out"></pre></section>';
    const load = async function () { document.getElementById("out").textContent = JSON.stringify(await request("GET", "/reports"), null, 2); };
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    try {
      ...
      if (PAGE === "installations") return installations();
      if (PAGE === "returns") return returnsPage();
      if (PAGE === "warranty") return warranty();
      if (PAGE === "reports") return reports();
      if (ACTIONS[PAGE]) return actionPage(ACTIONS[PAGE]);
      throw new Error("Unsupported Furniture page.");
    } catch (error) {
      if (!document.getElementById("app")) shell("Furniture Application", "Unable to open requested module");
      document.getElementById("app").innerHTML = '<section class="f-panel"><div class="f-status error">' + esc(error.message || "Furniture application failed") + "</div></section>";
    }
  });
})();
