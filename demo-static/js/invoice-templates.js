...
(function(){
  ...
  const templateList=[
    ...
    {id:'thermal-58',name:'Thermal Receipt 58mm',use:'Compact POS terminal receipt printer',size:'58mm',badge:'Terminal'},
  ...
  function getCompany(){ensure(); return Object.assign({},defaultCompany,read(KEYS.company,defaultCompany),cloudConfig&&cloudConfig.company||{})}
  function getInvoice(){ensure(); return Object.assign({},defaultInvoice,read(KEYS.invoice,defaultInvoice),cloudConfig&&cloudConfig.invoice||{})}
  function getDesigner(){ensure(); const d=Object.assign({},defaultDesigner,read(KEYS.designer,defaultDesigner),cloudConfig&&cloudConfig.designer||{});d.columns=Object.assign({},defaultDesigner.columns,d.columns||{});migrateDesignerColumns(d); return d}
  function setCloudConfig(config){cloudConfig=config&&typeof config==='object'?config:null}
  ...
  function visibleColumns(templateId){
    ...
    if(templateId==='thermal-80'||templateId==='thermal-58') return ['name','qty','rate','total'];
  ...
  function encodedMark(data,kind){
    const value=firstText(kind==='qr'?data.qrValue:data.barcodeValue,kind==='qr'?data.qrCode:data.barcode);
    const image=firstText(kind==='qr'?data.qrImageUrl:data.barcodeImageUrl);
    if(!value||!image) return '';
    return `<div class="${kind==='qr'?'qr-box':'barcode-box'}" style="height:auto;width:auto;padding:6px"><img src="${esc(image)}" alt="${kind==='qr'?'QR code':'Barcode'}" style="display:block;max-width:${kind==='qr'?'92px':'180px'};max-height:92px"><small>${esc(value)}</small></div>`;
  }
  ...
  function render(templateId, opts={}){
    ...
    const isThermal=templateId==='thermal-80'||templateId==='thermal-58';
    const cls=[isThermal?'thermal '+templateId:'',templateId==='compact-a4'?'compact':'',templateId==='minimal'?'minimal':'',templateId==='letterhead'?'letterhead':'',templateId==='bilingual'?'bilingual':''].join(' ');
    const title=data.documentTitle || (data.documentType==='quotation'?'Quotation':data.documentType==='delivery_note'?'Delivery Note':(templateId==='tax-invoice'?'Tax Invoice':templateId==='quotation'?'Quotation':templateId==='delivery-invoice'?'Delivery Note':isThermal?'Receipt':'Sales Invoice'));

    if(isThermal) return baseCss(des)+`<style>.thermal-80{width:76mm!important;max-width:76mm!important}.thermal-58{width:54mm!important;max-width:54mm!important}@media print{@page{size:${templateId==='thermal-58'?'58mm':'80mm'} auto;margin:2mm}}</style><div class="invoice-sheet ${cls}"><div class="inv-head">${brandBlock(c,inv)}<p>${esc(data.branch)}${data.counterName?`<br>${esc(data.counterName)}`:''}<br>${esc(data.invoiceNo)} • ${esc(data.time)}</p></div><p><b>Cashier:</b> ${esc(data.cashier)}${data.shiftReference?`<br><b>Shift:</b> ${esc(data.shiftReference)}`:''}<br><b>Customer:</b> ${esc(data.customer)}${data.lpoNo?`<br><b>LPO No:</b> ${esc(data.lpoNo)}`:''}</p>${tableHtml(data.items,templateId)}${totalsBlock(data)}<p><b>Payment:</b> ${esc(data.paymentMethod)}<br><b>Paid:</b> ${money(data.paid)}<br><b>Balance:</b> ${money(data.balance)}</p>${inv.showQr?encodedMark(data,'qr'):''}${footerBlock(c,inv,resolvedSalesman,data)}<p style="text-align:center">Thank you!</p></div>`;
    ...
    return baseCss(des)+`<style>@media print{@page{size:A4 portrait;margin:8mm}.invoice-sheet{width:194mm;min-height:281mm;margin:0 auto;padding:8mm;box-sizing:border-box}.inv-table thead{display:table-header-group}.inv-table tr{break-inside:avoid;page-break-inside:avoid}}</style><div class="invoice-sheet ${cls}"><div class="inv-head">${brandBlock(c,inv)}<div class="inv-title"><h1>${title}${templateId==='bilingual'?' / فاتورة':''}</h1><p><span class="copy-label">${esc(inv.copyLabel)}</span></p><p><b>${esc(data.invoiceNo)}</b><br>${esc(data.date)}</p></div></div>${bilingual}<div class="inv-box-grid"><div class="inv-box"><b>Customer / العميل</b><br>${esc(data.customer)}<br>${esc(data.deliveryAddress)}${inv.showCustomerBalance?`<br>Balance: ${money(data.customerBalance)}`:''}</div><div class="inv-box"><b>Sales Info</b><br>Branch: ${esc(data.branch)}${data.warehouseName?`<br>Warehouse: ${esc(data.warehouseName)}`:''}${data.counterName?`<br>Counter: ${esc(data.counterName)}`:''}<br>Cashier: ${esc(data.cashier)}<br>Salesperson: ${esc(resolvedSalesman||'-')}<br>Payment: ${esc(data.paymentMethod)}${data.lpoNo?`<br>LPO No: ${esc(data.lpoNo)}`:''}<br>Template: ${esc(t.name)}</div></div>${paint}${tax}${tableHtml(data.items,templateId)}<div style="display:flex;gap:20px;align-items:flex-start;justify-content:space-between"><div>${inv.showQr?encodedMark(data,'qr'):''}${inv.showBarcode?encodedMark(data,'barcode'):''}</div>${totalsBlock(data)}</div>${footerBlock(c,inv,resolvedSalesman,data)}</div>`;
  }
  ...
  window.AxtorInvoice={templates:templateList,render,preview,print,selectedTemplate,getCompany,getInvoice,getDesigner,setCloudConfig,toast,audit,sampleData,normalizeData,resolveSalesman};
