...
const p50GetWithProfile=get;
get=async function(path){
  let target=String(path||'');
  if(target.startsWith('/api/v1/grocery/print/document/')&&!target.includes('?')){
    const code=document.querySelector('#print50-form [name=profile]')?.value;
    if(code)target+=`?profile=${encodeURIComponent(code)}`;
  }
  const response=await p50GetWithProfile(target);
  if(target.startsWith('/api/v1/grocery/print/document/'))p50LastPrintProfile=unwrap(response)?.profile||null;
  return response;
};
