(() => {
  ...
  function recordAudit(action, entityType, entityId, metadata = {}) {
    ...
    window.dispatchEvent(new CustomEvent('axtor:audit', { detail: records[0] }));
    return records[0];
  }
  ...
  async function flushOfflineQueue() {
    ...
    for (const item of queue) {
      try {
        await api(item.path, { method: item.method || 'POST', body: JSON.stringify(item.body || {}) });
        processed += 1;
        recordAudit('OFFLINE_MUTATION_SYNCED', item.entityType || 'request', item.entityId, { queueId: item.id });
      } catch (error) {
        remaining.push({ ...item, attempts: (item.attempts || 0) + 1, lastError: error.message });
      }
    }
  ...
  function shareEmail({ to = '', subject = 'Axtor POS document', body = '', documentUrl = '' }) {
    const mailto = `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent([body, documentUrl].filter(Boolean).join('\n'))}`;
