(function () {
  ...
  var state = { products: [], customers: [], salesPersons: [], counters: [], currentShift: null, selectedCounter: null, context: {}, cart: [], drafts: [], currentDraftId: null, lastDocument: null };
  ...
  async function load() {
    ...
    var terminalContext = await Promise.allSettled([
      U.api().apiGet("/api/v1/branches/counters"),
      U.api().apiGet("/api/v1/shifts/current")
    ]);
    state.counters = terminalContext[0].status === "fulfilled" ? normalizeList(terminalContext[0].value) : [];
    state.currentShift = terminalContext[1].status === "fulfilled" ? (U.data(terminalContext[1].value) || null) : null;
    state.selectedCounter = resolveCounter();
    renderAll();
  }

  function resolveCounter() {
    if (state.currentShift && state.currentShift.counter) return state.currentShift.counter;
    var currentUser = state.context.currentUser || {};
    var branchId = (state.currentShift && state.currentShift.branchId) || currentUser.branchId || "";
    return state.counters.find(function (row) { return row.id === (state.currentShift && state.currentShift.counterId); }) ||
      state.counters.find(function (row) { return row.cashierUserId === currentUser.id && String(row.status || "ACTIVE").toUpperCase() === "ACTIVE"; }) ||
      state.counters.find(function (row) { return row.branchId === branchId && String(row.status || "ACTIVE").toUpperCase() === "ACTIVE"; }) ||
      state.counters.find(function (row) { return String(row.status || "ACTIVE").toUpperCase() === "ACTIVE"; }) || null;
  }
  ...
  function renderTerminalContext() {
    ...
    var counter = state.selectedCounter;
    var branchId = (shift && shift.branchId) || (counter && counter.branchId) || (state.context.currentUser && state.context.currentUser.branchId);
    var branch = (shift && shift.branch) || (counter && counter.branch) || (state.context.branches || []).find(function (row) { return row.id === branchId; });
    var cashier = (shift && shift.cashierName) || (state.context.currentUser && state.context.currentUser.name) || "Current user";
    var counterName = (shift && (shift.counterName || (shift.counter && shift.counter.name))) || (counter && counter.name) || "Not assigned";
    var shiftText = shift ? "OPEN · " + U.datetime(shift.openedAt) : "No open shift";
    setText("terminalBranchName", branch && branch.name || "Not assigned");
    setText("terminalCounterName", counterName);
    setText("terminalCashierName", cashier);
    setText("terminalShiftStatus", shiftText);
    setText("terminalCounterBadge", counterName);
    var badge = U.q("#terminalCounterBadge");
    if (badge) badge.className = "badge-soft " + (shift ? "badge-paid" : "badge-pending");
  }
  ...
  function payload(postingMode) {
    ...
    return {
      ...
      branchId: (state.currentShift && state.currentShift.branchId) || (state.selectedCounter && state.selectedCounter.branchId) || (state.context.currentUser && state.context.currentUser.branchId) || null,
      counterId: (state.currentShift && state.currentShift.counterId) || (state.selectedCounter && state.selectedCounter.id) || null,
  ...
  U.run(async function () {
    ...
    U.bind("#printTerminalReceiptBtn", "click", function () {
      if (!state.lastDocument || !state.lastDocument.id) return U.toast("Complete a sale before printing its invoice", "error");
      window.open("invoice-view.html?id=" + encodeURIComponent(state.lastDocument.id), "_blank");
    });
