...
(function () {
  ...
  function validatePayload(payload, postingMode) {
    ...
    const outstanding = Math.max(0, number(payload.grandTotal) - paid);
    if (outstanding > 0 && !payload.customerId) errors.push("Customer is required for a credit sale.");
    if (outstanding > 0 && !payload.dueDate) errors.push("Due date is required for a credit sale.");
